...
async def createNote(args, messageWrapper: DiscordMessageWrapper):
    ...
    # Write the note
    bundle = PartitionMgr.Data.appdata() + f"/jot-notes/{title}.jnote"
...
async def mainAsync(args, message: discord.Message):
    try:
        ...
        if not os.path.exists(PartitionMgr.Data.appdata() + "/jot-notes"):
            os.mkdir(PartitionMgr.Data.appdata() + "/jot-notes")
        ...
        if args[0] == "list":
            notes = os.listdir(PartitionMgr.Data.appdata() + "/jot-notes")
