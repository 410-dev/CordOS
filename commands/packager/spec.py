...
class Spec:
    def __init__(self, path, data=None):
        ...
        # Check compatibility
        installerSpecVersion = int(Registry.read("SOFTWARE.NanoPyOS.Packages.Compatibility.InstallerSpecVersion", default=1))
        systemSupportedSDKMinimum = int(Registry.read("SOFTWARE.NanoPyOS.Packages.Compatibility.SDKMinimum", default=1))
        systemSupportedSDKMaximum = int(Registry.read("SOFTWARE.NanoPyOS.Packages.Compatibility.SDKMaximum", default=1))

        if self.spec is None or self.specv is None or self.sdk is None:
            raise Exception("Invalid spec file. (Missing fields)")

        if self.spec != "NanoPyOSInstallablePackage":
