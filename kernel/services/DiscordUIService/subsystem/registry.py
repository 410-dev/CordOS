...
def getRegloc() -> str:
    containerPath = Isolation.getCallerContainerPath()
    containerPath += "/etc/registry"
    return containerPath

def read(key: str, default=None, writeDefault=False):
    regloc = getRegloc()
    key = key.replace(".", "/")
...
def write(key: str, value=None, verbose=False):
    regloc = getRegloc()
    key = key.replace(".", "/")
...
def delete(key: str, deleteSubkeys: bool = False):
    regloc = getRegloc()
    key = key.replace(".", "/")
    if not os.path.exists(os.path.join(regloc, key)):
        ...
        if deleteSubkeys:
            for file in os.listdir(os.path.join(regloc, key)):
                ...
                if os.path.isfile(os.path.join(regloc, key, file)):
                    ...
                    delete(os.path.join(key, file), deleteSubkeys)
        os.rmdir(os.path.join(regloc, key))
        journal("delete", True, "<none>", key, "<none>")


def listSubKeys(key: str, subdirectories: list = []) -> list:
    regloc = getRegloc()
    key = key.replace(".", "/")
    if not os.path.exists(os.path.join(regloc, key)):
    ...
    for file in os.listdir(os.path.join(regloc, key)):
        if os.path.isfile(os.path.join(regloc, key, file)) and not file.startswith("."):
            ...
            listSubKeys(os.path.join(key, file), subdirectories)

    journal("subkeys", True, "<none>", key, "<list=content>" if len(listOfFiles) > 0 else "<list=empty>")
    return listOfFiles


def build(blueprintPath: str, silent=False):
    lines: list = []
    ...
    for line in lines:
        ...
        write(key, value, verbose=not silent)
