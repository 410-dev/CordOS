...
def install(keywords: list, case: str, url=False):
    ...
    try:
            ...
            for instructionObject in procedure:
                ...
                if task == "install":
                    try:
                        ...
                        # Create receipt
                        def recursiveFSSearch(path: str, traces: list) -> list:
                            for item in os.listdir(path):
                                if os.path.isdir(os.path.join(path, item)):
                                    traces = recursiveFSSearch(os.path.join(path, item), traces)
                                else:
                                    if item.startswith("."):
                                        os.remove(os.path.join(path, item))
                                        continue
                                    traces.append(os.path.join(path, item))
                            return traces
                        ...
                        if not foundSpec:
                            Journaling.record("ERROR", f"Package {package}@{spec.getName()} does not contain spec.json, skipping.")
                            IO.println(f"Error: Package {package}@{spec.getName()} does not contain spec.json, skipping.")
                            installFailed.append(spec.getName())
                            break
        ...
        # Clean up
        Journaling.record("INFO", "Cleaning up...")
    ...
    if len(installFailed) > 0:
        IO.println("Error: Failed to install packages:")
        ...
        Journaling.record("INFO", "Packages installed successfully.")
