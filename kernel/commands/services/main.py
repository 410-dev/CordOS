            ...
            try:
                ...
                if os.path.isfile(moduleName.replace(".", "/") + ".py"):
                    with open(moduleName.replace(".", "/") + ".py", 'r') as f:
                        if "async def mainAsync(" not in f.read():
                            self.message.reply(f"User service '{self.args[2]}' does not have a configure function in configure module.", mention_author=True)
                            return
                ...
                try:
                    ...
                    if os.path.isfile(moduleName.replace(".", "/") + ".py"):
                        with open(moduleName.replace(".", "/") + ".py", 'r') as f:
                            if "async def mainAsync(" not in f.read():
                                self.message.reply(f"Kernel service '{self.args[2]}' does not have a configure function in configure module.", mention_author=True)
                                return
                    module = importlib.import_module(moduleName)
